...
def plot_hasse(subplot, abstractions, title='', loga=False):
    ...
    lb = lower_bound_diamond(len(abstractions[0]))
    ub = upper_bound_diamond(len(abstractions[0]))

    # 2. Plot
    x_p, y_p = positions_on_grid(abstractions)
    ...
    subplot.plot(x_p,y_p,color="red")                   # Plots tracectory

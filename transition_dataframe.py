...
def transitions_dataframe_from_rndws(rndws, x):
    ...
    nrows = df.shape[0]
    df = df.assign(change_local=[0 for i in range(nrows)], change_global=[0 for i in range(nrows)])
    for i in range(nrows):
        df.loc[i, 'change_local']     = sos.change_local(sos.from_string(df.loc[i, 'a_1']), sos.from_string(df.loc[i, 'a_2']))
        df.loc[i, 'change_global']    = sos.change_global(sos.from_string(df.loc[i, 'a_1']), sos.from_string(df.loc[i, 'a_2']))
...
def abstractions_dataframe_from_rndws_and_transitions_dataframe(rndws, x, transitions_df):
    ...
    abstractions = list(set(abstractions))
...
def data_from_json(json_path, x, save=False):
    '''
    ...
    if('/' in json_path):
        ...
        json_path = json_path.split('\\')
    file_name = json_path[len(json_path)-1]
    directory = json_path[0]
    for i in range(1,len(json_path)-1):
        directory = directory+'/'+json_path[i]
